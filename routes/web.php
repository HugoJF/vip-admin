 ...
 */
Route::group(['middleware' => ['accepted', 'auth']], function () {
    Route::get('pp-orders/create', 'PayPalController@create')->name('pp-orders.create')->middleware('can:create,App\Order');
    Route::post('pp-orders/store', 'PayPalController@store')->name('pp-orders.store')->middleware('can:create,App\Order');
...
Route::group(['middleware' => ['accepted', 'auth']], function () {
    Route::get('mp-orders/{order}/recheck', 'MPOrderController@recheck')->name('mp-orders.recheck');
    ...
    Route::get('mp-orders/{order}', 'MPOrderController@show')->name('mp-orders.show')->middleware('can:view,order');

    Route::group(['middleware' => ['admin']], function () {
        Route::get('/mp_preferences/{id}', function ($id) {
            return view('pre', [
                'title'   => 'MercadoPago Preference API Information',
                'content' => json_encode(MP::get_preference($id)),
            ]);
        })->name('mp-orders.debug.preference');

        Route::get('mp_payments/{id}', function ($id) {
            return view('pre', [
                'title'   => 'MercadoPago Payment API Information',
                'content' => json_encode(MP::get_payment($id)),
            ]);
        })->name('mp-orders.debug.payment');

        Route::get('mp_merchant_order/{id}', function ($id) {
            return view('pre', [
                'title'   => 'MercadoPago Merchant Order API Information',
                'content' => json_encode(MP::get('/merchant_orders/'.$id)),
            ]);
        })->name('mp-orders.debug.order');
    });
});
